...
def collect():
    ...
    try:
        for key in filter_keys:
            cmd_res = commands.getoutput('dmidecode -t system | grep "%s"' % key).strip()
...
def cpuinfo():
    ...
    cpu_model_list = raw_data["cpu_model"].split(":")

    if len(cpu_model_list) >1:
        data["cpu_model"] = cpu_model_list[1].strip()
    else:
        data["cpu_model"] = -1
